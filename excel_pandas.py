    ...
    for i in range(0, 19):
        while(str(date1)[i]!=str(date2)[i]):
            if(int(str(date1)[i])>int(str(date2)[i])):
                print("File1 is derived from File2. (According to " + file1["FIRST NAME"][ind1] + " " + file1["LAST NAME"][ind1] + "'s "+ col +")")
                return
            else:
                print("File2 is derived from File1. (According to " + file1["FIRST NAME"][ind1] + " " + file1["LAST NAME"][ind1] + "'s "+ col +")")
                return
